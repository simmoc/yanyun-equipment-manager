...
import type { EquipmentSlot, SuitType } from '@/types';

type Attribute = { name: string; value: number; is_main: boolean };
...
type NewEquipmentModalProps = ModalProps & {
  equipmentData: NewEquipmentData;
  setEquipmentData: React.Dispatch<React.SetStateAction<NewEquipmentData>>;
  affixMode: 'pve' | 'pvp';
  ...
  return (
    <ModalOverlay>
      <div className="bg-gray-800 p-6 rounded-lg modal-enter max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4">新建装备</h2>
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4"
            />
            <label htmlFor="isWearing" className="text-gray-400">立即装备</label>
          </div>
                ...
                <div key={index} className="flex gap-2">
                  ...
                  {index < 5 && (
                    <input
                      ...
                      onChange={(e) => {
                        const newAttrs = [...equipmentData.attributes];
                        newAttrs[index].value = parseInt(e.target.value) || 0;
                        setEquipmentData({ ...equipmentData, attributes: newAttrs });
                      }}
