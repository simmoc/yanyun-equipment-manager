...
def print_menu(has_exe: bool = False):
    ...
    if HAS_PROXY_MANAGER:
        print("  [1] mitmweb + 自动设置系统代理 (端口8080)")
        print("  [2] mitmweb + 自动设置系统代理 (端口9000)")
        print("  [3] mitmweb + 自动设置系统代理 (自定义端口)")
        print("  [4] mitmweb - 可视化界面（不自动代理）")
    else:
        if has_exe:
    ...
    print("  [5] mitmdump - 后台监听模式")
    
    if has_exe:
        print("  [6] 运行证书安装工具 (EXE)")
    
    print("  [7] 查看最新捕获的数据")
    print("  [8] 查看所有历史数据")
    print("  [9] 安装mitmproxy证书")
    
    if HAS_PROXY_MANAGER:
        print("  [10] 设置系统代理 (127.0.0.1:8080)")
        print("  [11] 设置系统代理 (127.0.0.1:9000)")
        print("  [12] 恢复/禁用系统代理")
...
def start_web_v2(with_proxy: bool = False, port: int = 8080):
    print("=" * 60)
    if with_proxy:
        print(f"启动 mitmweb + 自动系统代理（端口{port}）...")
    else:
    ...
    print("  1. 启动后会自动打开浏览器")
    if not with_proxy:
        print(f"  2. 在浏览器中配置代理: 127.0.0.1:{port}")
    else:
        print(f"  2. 系统代理已自动设置为 127.0.0.1:{port}")
    ...
    try:
        subprocess.run(["mitmweb", "-p", str(port), "-s", "network_monitor_final.py"])
...
def set_system_proxy(port: int = 8080):
    """设置系统代理"""
    ...
    print("=" * 60)
    print(f"设置系统代理 (127.0.0.1:{port})")
    print("=" * 60)
    print("\n这将把系统代理设置为 127.0.0.1:{port}")
    print("停止时会自动恢复原来的设置")
    print()
...
def start_dump(port: int = 8080):
    print("=" * 60)
    print("启动 mitmdump 后台监听...")
    print("=" * 60)
    print("提示:")
    print("  1. 启动后程序会在后台运行")
    print(f"  2. 需要在浏览器中配置代理: 127.0.0.1:{port}")
    print("  3. 登录燕云游戏并查看角色信息")
    print("  4. 程序会自动捕获并解析数据")
    print("  5. 按 Ctrl+C 停止监听")
    print("=" * 60)
    ...
    try:
        subprocess.run(["mitmdump", "-p", str(port), "-s", "network_monitor_v2.py"])
    except KeyboardInterrupt:
        print("\n已停止监听")
    except Exception as e:
        print(f"✗ 启动失败: {e}")
...
def main():
    ...
    while True:
        ...
        if HAS_PROXY_MANAGER:
            if choice == '1':
                start_web_v2(with_proxy=True, port=8080)
            elif choice == '2':
                start_web_v2(with_proxy=True, port=9000)
            elif choice == '3':
                port_input = input("请输入端口号 (默认8080): ").strip()
                port = int(port_input) if port_input.isdigit() else 8080
                start_web_v2(with_proxy=True, port=port)
            elif choice == '4':
                start_web_v2(with_proxy=False)
            elif choice == '5':
                start_dump()
            elif choice == '6' and has_exe:
                ...
                set_system_proxy(port=8080)
            elif choice == '11':
                set_system_proxy(port=9000)
            elif choice == '12':
