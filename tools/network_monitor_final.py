...
def load_affix_config():
    """加载词缀配置"""
    global affix_data
    affix_config_path = os.path.join(CONFIG_DIR, "affix_data.json")
    if os.path.exists(affix_config_path):
        try:
            with open(affix_config_path, 'r', encoding='utf-8') as f:
                affix_data = json.load(f)
            ctx.log.info(f"✓ 加载词缀配置: {len(affix_data)} 个词缀")
        except Exception as e:
            ctx.log.error(f"✗ 加载词缀配置失败: {e}")
            affix_data = {}
...
def build_character_info(data):
    ...
    character_info = {
        ...
        'character_info': {
            'base': {
                ...
                'number_id': data.get('base.number_id', ''),
                'level': data.get('base.level', 0),
                'school': get_school_name(data.get('base.school', 0)),
                'school_id': data.get('base.school', 0),
                'max_xiuwei_kungfu': data.get('base.max_xiuwei_kungfu', 0),
                'create_time': format_timestamp(data.get('base.create_time', 0)),
                'login_time': format_timestamp(data.get('base.login_time', 0)),
                'online_time': format_duration(data.get('base.online_time', 0))
            },
            'real_attr': {},
            'equipments': []
        }
    ...
    for key in data:
        if key.startswith('real_attr.'):
            attr_key = key.split('.')[-1]
            character_info['character_info']['real_attr'][get_attr_name(attr_key)] = data[key]
    ...
    for slot, equip_data in wear_equips.items():
        ...
        equip_info = {
            'slot': slot,
            'no': equip_no,
            'name': get_equip_name(equip_no),
            'suffix': get_suffix_name(suffix_id),
            'suffix_id': suffix_id,
        ...
        character_info['character_info']['equipments'].append(equip_info)
    ...
    def response(self, flow: http.HTTPFlow):
        if TARGET_URL in flow.request.pretty_host and TARGET_PATH in flow.request.path:
            try:
                ...
                if raw_data.get('success') and raw_data.get('code') == 0:
                    data = raw_data.get('data', {})
                    
                    if not data:
                        ctx.log.warn("[!] data 字段为空")
                        return
                    
                    # 先保存原始数据
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    raw_file = f"{SAVE_DIR}/raw_{timestamp}.json"
                    with open(raw_file, 'w', encoding='utf-8') as f:
                        json.dump(raw_data, f, ensure_ascii=False, indent=2)
                    ctx.log.info(f"✓ 原始数据已保存: {raw_file}")
                    
                    # 打印角色信息
                    print_character_info(data)
                    
                    # 保存解析后的数据
                    character_info = build_character_info(data)
                    output_file = f"{SAVE_DIR}/character_{timestamp}.json"
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump(character_info, f, ensure_ascii=False, indent=2)
                    ctx.log.info(f"✓ 解析数据已保存: {output_file}")
                    
                    # 保存最新数据副本
                    latest_file = f"{SAVE_DIR}/latest_character.json"
